...
# Aula 08 - CRIAÇÃO DE RELATÓRIOS NO R
# =========================================================
...
# shiny - Criar aplicações web interativas diretamente no R
# shinythemes - Aplicar temas visuais prontos em aplicativos Shiny
# shinyWidgets - Adicionar widgets modernos e componentes avançados ao Shiny
# DT - Criar tabelas interativas com filtros, busca e exportação
...
# sidrar - Baixar dados oficiais do IBGE utilizando a API SIDRA
# bslib - Personalizar temas Bootstrap em aplicativos Shiny e relatórios
# htmltools - Criar e manipular elementos HTML no R
# scales - Formatar escalas, números, porcentagens e eixos de gráficos
# ggplot2 - Criar gráficos estatísticos e visualizações profissionais
# knitr - Gerar tabelas e relatórios dinâmicos no R Markdown e Quarto
# tidyr - Organizar e estruturar dados em formatos longos e largos
# quarto - Criar relatórios, dashboards e apresentações automatizadas
...
server <- function(input, output, session) {
  ...
  output$download_relatorio_html <- downloadHandler(
    filename = function() {
      paste0("relatorio_ibge_", Sys.Date(), ".html")
    },
    content = function(file) {
      
      req(dados_filtrados())
      
      arquivo_qmd_original <- normalizePath(
        "relatorio_ibge.qmd",
        mustWork = FALSE
      )
      
      if (!file.exists(arquivo_qmd_original)) {
        stop("O arquivo relatorio_ibge.qmd não foi encontrado na pasta do app.")
      }
      
      temp_dir <- tempdir()
      temp_qmd <- file.path(temp_dir, "relatorio_ibge.qmd")
      output_name <- "relatorio_ibge_render.html"
      output_path <- file.path(temp_dir, output_name)
      
      file.copy(
        from = arquivo_qmd_original,
        to = temp_qmd,
        overwrite = TRUE
      )
      
      dados_path <- file.path(temp_dir, "dados_filtrados.rds")
      saveRDS(dados_filtrados(), dados_path)
      ...
      quarto::quarto_render(
        ...
        execute_params = list(
          ...
          titulo_relatorio = "Relatório Executivo de Dados do IBGE",
          subtitulo_relatorio = "Análise automatizada com Quarto + R + SIDRA"
        ),
        quiet = FALSE
      )
